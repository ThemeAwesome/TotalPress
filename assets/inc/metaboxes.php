...
class TotalPress_Page_Options_Meta_Box {
	...
	public function render_page_options_metabox( $post ) {
		...
		wp_nonce_field('totalpress_nonce_action','totalpress_nonce');
		// Retrieve an existing value from the database.
		$page_options_hide_post_title = get_post_meta($post->ID,'page_options_hide_post_title',true);

		// Set default values.

		// Form fields.
		echo '<p><label><input type="checkbox" name="page_options_hide_post_title" class="page_options_post_title__field" value="' . $page_options_hide_post_title . '" ' . checked( $page_options_hide_post_title,'checked',false) . '> ' . '</label>';
	...
	public function save_metabox( $post_id, $post ) {
		...
		if ( wp_is_post_revision( $post_id ) )
			return;
		// Sanitize user input.
		$page_options_new_hide_post_title = isset( $_POST[ 'page_options_hide_post_title' ] ) ? 'checked' : '';
		// Update the meta field in the database.
		update_post_meta( $post_id, 'page_options_hide_post_title', $page_options_new_hide_post_title );
	}
}
